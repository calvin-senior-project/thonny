...
from thonny.plugins.codelive.utils import get_instruction, get_new_id, free_id
...
class Session:
    def __init__(self, name = None, is_host = False, is_cohost = False):
    ...
    def bind_keypress(self):
        ...
        bind_hash["<KeyPress>|self._editor_notebook.get_current_editor()"] = text_widget.bind("<Key>", self.broadcast_keypress, True)
        # bind_hash["<BackSpace>|self._editor_notebook.get_current_editor().get_text_widget()"] = WORKBENCH.bind("<>", self.broadcast_keypress, True)
        bind_hash["<<TextChange>>"] = text_widget.bind("<Meta-Delete>", self.bounce, True)
        return bind_hash
